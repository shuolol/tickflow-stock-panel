; ===========================================================================
; StrategyMind — Inno Setup 安装包脚本
; ===========================================================================
; 用途: 把 PyInstaller 产出的 dist/StrategyMind/ 文件夹封装成
;       单个 Setup.exe 安装程序 (双击→安装向导→快捷方式→可卸载)。
;
; 构建 (本地):
;   1. 先跑 PyInstaller: cd backend && uv run pyinstaller ../packaging/strategymind.spec
;   2. 再跑 Inno Setup:   ISCC.exe packaging\strategymind.iss
;   3. 产物: packaging\Output\StrategyMind-Setup-x.x.x.exe
;
; 设计决策:
...
#define MyAppName          "StrategyMind"
#define MyAppNameEN       "StrategyMind"
#define MyAppExeName      "StrategyMind.exe"
#define MyAppPublisher    "StrategyMind"
...
DefaultDirName=D:\StrategyMind
DefaultGroupName={#MyAppName}
DisableProgramGroupPage=yes
OutputDir=Output
OutputBaseFilename=StrategyMind-Setup-{#MyAppVersion}
...
Source: "..\backend\dist\StrategyMind\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
begin
  ...
  begin
    DefaultDir := ExpandConstant('{localappdata}\Programs\StrategyMind');
